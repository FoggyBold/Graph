...
namespace Graph.Forms
{
    ...
    {
        private ReadTwoDigits readTwoDigits;
        private double X { get; set; }
        private double Y { get; set; }
        public Form4(Form2 form2) : base(form2)
        {
            InitializeComponent();
            readTwoDigits = new ReadTwoDigits(this) { Visible = false };
            X = Y = 0;
        }
        ...
        private void updateLength(Line line)
        {
            ...
            line.updateLength(Math.Round(length * X / Y, 1));
        }
        ...
        protected override void Form1_MouseDown(object sender, MouseEventArgs e)
        {
                        ...
                        if (secondNodeForConnection != null && currNodeForConnection != secondNodeForConnection)
                        {
                            ...
                            else
                            {
                                ...
                                if (check != null)
                                {
                                    ...
                                    currNodeForConnection.Connection.Add(new Tuple<Node, double>(secondNodeForConnection, Math.Round(length * X / Y, 1)));
                                    Lines.addLine(new Line(currNodeForConnection, secondNodeForConnection, this.BackColor != Color.Black ? Color.White : Color.Black, Math.Round(length * X / Y, 1)));
                                    ((Control)sender).Invalidate();
                                }
                                else
                                {
            ...
            {
                if (readTwoDigits != null)
                {
                    readTwoDigits.Visible = true;
                }
                else
                {
                    readTwoDigits = new ReadTwoDigits(this);
                    readTwoDigits.Visible = true;
                }
            }
        }
        ...
        public void closeReadTwoDigitsForm()
        {
            readTwoDigits = null;
        }
    }
